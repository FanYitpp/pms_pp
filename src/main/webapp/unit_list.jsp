...
<script type="text/javascript">
	$(document).ready(function(){
	    ...
	    $("#addBtn").fancybox({
	        ...
	        'onClosed' : function() { 
	        	window.location.href = '<%=basePath %>initunit.action?builid=${builid}';
	        }
	    });
	    ...
	    sessionStorage.setItem('builid', '${builid}');
			...
			<div class="ui_content">
				<div class="ui_tb">
					<table class="table" cellspacing="0" cellpadding="0" width="100%" align="center" border="0">
						...
						<c:forEach items="${ALunit }" var="unit">
							<tr>
								<td><input type="checkbox" name="IDCheck" value="14458579642011" class="acb" /></td>
								<td>${unit.unitName }</td>
								<td>${unit.unitFloorNum }</td>
								<td><a href="inithouse.action?unitid=${unit.unitId }">查看房屋</a> </td>
								<td>
									<a href="<%=basePath %>initeditunit.action?unitid=${unit.unitId }" class="edit">编辑</a> 
									<a href="<%=basePath %>delectunit.action?unitid=${unit.unitId }&builid=${builid }">删除</a>
								</td>
							</tr>
						</c:forEach>
