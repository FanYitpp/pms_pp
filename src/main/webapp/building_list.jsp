...
<script type="text/javascript">
	$(document).ready(function(){
	    ...
	    $("#addBtn").fancybox({
	        ...
	        'onClosed' : function() { 
	        	window.location.href = '<%=basePath %>initbuil.action?commid=${commid}';
	        }
	    });
			...
			<div class="ui_content">
				<div class="ui_tb">
					<table class="table" cellspacing="0" cellpadding="0" width="100%" align="center" border="0">
						...
						<c:forEach items="${ALbuil }" var="buil">
							<tr>
								<td><input type="checkbox" name="IDCheck" value="14458579642011" class="acb" /></td>
								<td>${buil.buildingName }</td>
								<td>${buil.buildingUnitnum }</td>
								<td>${buil.buildingFloornum }</td>
								<td>${buil.buildingCreatetime }</td>
								<td><a href="initunit.action?builid=${buil.buildingId }">查看单元</a> </td>
								<td>
									<a href="<%=basePath %>initeditbuil.action?builid=${buil.buildingId }" class="edit">编辑</a> 
									<a href="<%=basePath %>delectbuil.action?builid=${buil.buildingId }&commid=${commid }">删除</a>
								</td>
							</tr>
						</c:forEach>
